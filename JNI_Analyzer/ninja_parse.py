...
# project_path = '/Volumes/android/android-8.0.0_r34'
...
def process_soong_ninja():
    ...
    while True:
        ...
        if line:
            if not line.strip().startswith('#'):
                line = line.replace('\n', '')
                # print(line)
                if('m.libFraunhoferAAC_android_arm64_armv8-a_static_core.moduleDescSuffix' in line):
                    pppp=0
                ...
                # rule模式结束
                    ...
                    rule_mode = False
                    # rule_small['command'] = process_ninja(rule_small['command'], global_dic)
                ...
                # build模式结束
                    ...
                    if rule_name .startswith('g.cc'):
                        ...
                        # if rule_name == 'phony':
                        #     rule_command = 'phony'
                        # elif rule_name == 'g.python.par':
                        #     rule_command = 'g.python.par'
                    ...
                    # if 'android_media_MediaDrm.cpp' in rule_command:
                    # print('======start to find======')
                    # print(rule_command)
                    ...
                    while len(rule_command_split)>0:
                        if ('bin/clang' in rule_command_split[0] or 'bin/clang++' in rule_command_split[0]):
                            ...
                            content = {'directory': project_path, 'arguments': rule_command_split, 'file': rule_command_split[-1]}
                ...
                # rule中的配置项
                    ...
                    if '=' in line:
                        ...
                        pass
                # 结束rule
                # elif (not line.startswith(' ') and not line.startswith('\t')) and rule_mode:
                #     rule_mode = False
                #     # rule_small['command'] = process_ninja(rule_small['command'], global_dic)
                #     rule_dic_all[rule_name] = rule_small
                ...
                # elif (not line.startswith(' ') and not line.startswith('\t')) and build_mode:
                #     '''
                #     build output0 output1 | output2 output3: rule_name $
                #             input0 input1 $
                #             | input2 input3 $
                ...
                #     rule_name = build_cache[1].strip().split(' ', 1)[0]
                #     build_input = build_cache[1].strip().split(' ', 1)[1]
                #     rule_command = rule_dic_all[rule_name]
                #     build_output_explicit = build_output.strip().split('|', )[0]
                #     build_output_implicit = build_output.strip().split('|', )[1]
                #     build_input_implicit_order_only = build_input.strip().split('||')[1]
                #     build_input_explicit = build_input.strip().split('||')[0].strip().split('|', )[0]
    ...
    with open('tem/10.0out_build_ninja.json', 'w') as file_obj:
...
def process_out_ninja():
        ...
        if line:
            ...
            if '#' not in line and line.startswith('command') and (line.endswith('.cpp"') or line.endswith('.c"')):
                ...
                while '\$$(' in command:
                    a = re.findall('\$\((.*?)\)', command)
                    for tem in a:
                        # cat_command = tem.replace('\$$(cat ', '').replace(')', '')
                ...
                # compiler = command[0].strip()
                # flags = command[1].strip()
                # flags = flags.split()
                # source = flags[-1]
                # flags = flags[:-1]
                # if 'PWD=/proc/self/cwd' in compiler:
                #     print(command)
                #     print('=======compiler======')
                #     print(compiler)
                #     print('=======source======')
                ...
                #     print(flags)
                #     exit(2)
                # if 'frameworks/native/opengl/libagl/egl.cpp' in source:
                #     iiiii=0
                ...
                while len(command) > 0:
                    if ('bin/clang' in command[0] or 'bin/clang++' in command[0]):
                        ...
                        content = {'directory': project_path, 'arguments': command, 'file': source}
                        json_list.append(content)
                        break
                    else:
                        # print('======-1======')
    ...
    with open('tem/10.0build-aosp_arm64.json', 'w') as file_obj:
        json.dump(json_list, file_obj)

def merge_jsons(version):
    json_list = []

    files = set()
    fp = open('tem/'+version+'compile_commands.json', 'r')
    a = json.load(fp)

    fp = open('tem/'+version+'out_build_ninja.json', 'r')
    b = json.load(fp)

    fp = open('tem/'+version+'build-aosp_arm64.json', 'r')
    c = json.load(fp)

    for tem in a:
        if tem['file'] not in files:
            files.add(tem['file'])
            json_list.append(tem)
    ...
    with open('tem/'+version+'compile_commands_full.json', 'w') as file_obj:
        json.dump(json_list, file_obj)
